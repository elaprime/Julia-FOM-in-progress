...
n = length(inputs.radials);
bathyDepths = Vector{Vector{Any}}(undef,n); #needs to be dynamic

for n = 1:1:n
    bathyDepths[n] = ones(1);
end

ranges = [];

bathyVector = Vector{Vector{Any}}(undef,n); #needs to be dynamic

for m = 1:1:n
    bathyVector[n] = ones(1)
end
...
env = UnderwaterEnvironment(
    ...
    bathymetry = SampledDepth(1.0:13.74656335916021:55000, bathyDepths[1])
)

pm = Bellhop(env; gaussian=true);
tx = AcousticSource(0.0,-50.0,100.0);
rx = AcousticReceiverGrid2D(1,50,1000,-5000,25,200);
x = transmissionloss(pm,tx,rx);
plot(env;receivers=rx,transmissionloss=x, clims=(-120,-30))
